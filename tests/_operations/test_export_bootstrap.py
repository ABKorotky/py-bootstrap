...
class BaseExportBootstrapOperationTestCase(TestCase):
    tst_cls = ExportBootstrapOperation
    tst_obj: ExportBootstrapOperation
    ...
    def tearDown(self):
        shutil.rmtree(os.path.join(os.getcwd(), "test-destination"), ignore_errors=True)
    ...
    def test_run(self):
        ...
        self.tst_obj.set_bootstrap_path(path=self.templates_path / "test_bootstrap")
        ...
        assert os.path.isfile(
            os.path.join(destination_path, f"{self.tst_obj.entry_point_module_name}.py")
        )
